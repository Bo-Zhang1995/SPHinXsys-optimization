...
protected:
	...
	void update(size_t index_i, Real dt)
	{
		...
		if (pos_[index_i][1] < 0 && pos_[index_i][0] > 0.4 * L && pos_[index_i][0] < 0.6 * L)
		{
...
int main()
{
	...
	Real initial_learning_rate = 0.0002;
	...
	/************************************************************************/
	/* Solve diffusion PDE equation under optimized condition. */
	...
	std::ofstream out_file_all_information(filefullpath_all_information.c_str(), std::ios::app); //give the detailed information of optimization process.
		
	std::string filefullpath_error_plot = io_environment.output_folder_ + "/" + "error_plot.dat";
	std::ofstream out_file_error_plot(filefullpath_error_plot.c_str(), std::ios::app); //record various residue for visualization directly.
	
	std::string filefullpath_learning_rate = io_environment.output_folder_ + "/" + "learning_rate.dat";
	std::ofstream out_file_learning_rate(filefullpath_learning_rate.c_str(), std::ios::app); //record learning rate for each loop.

	std::string filefullpath_eta_regularization = io_environment.output_folder_ + "/" + "eta_regularization.dat";
	std::ofstream out_file_eta_regularization(filefullpath_eta_regularization.c_str(), std::ios::app); //record the regularization coefficient for each loop.
	...
	std::ofstream out_file_ite_T(filefullpath_ite_T.c_str(), std::ios::app); //record the steps for solving PDE equations.

	std::string filefullpath_ite_k = io_environment.output_folder_ + "/" + "ite_k.dat";
	std::ofstream out_file_ite_k(filefullpath_ite_k.c_str(), std::ios::app); //record the steps for evolution parameters.

	std::string filefullpath_residual_for_comparison = io_environment.output_folder_ + "/" + "residual_for_comparison.dat";
	std::ofstream out_file_residual_for_comparison(filefullpath_residual_for_comparison.c_str(), std::ios::app); //record the residue only related to the PDE solving.
	...
	while ((relative_temperature_difference > 0.00001 || averaged_residual_T_current_global > 0.000005 || 
		    relative_average_variation_difference > 0.00001) && ite_loop < 10000)
	{
		...
		/* Impose objective function and PDE error may increase. */
		...
		// The real temperature is not modified.
		//update_temperature_pde_residual.parallel_exec(dt);
		//averaged_residual_T_current_global = calculate_temperature_global_residual.parallel_exec(dt);
		//maximum_residual_T_current_global = calculate_maximum_error.parallel_exec(dt);
		...
		//	Parameter splitting.
		//----------------------------------------------------------------------
		/* Parameter splitting should return back the increased residual by imposing objective function. */
		...
		while (ite_k < ite_k_total)
		{
			...
			//	Regularization.
			//----------------------------------------------------------------------
			//if((double)rand() / RAND_MAX < 0.1 || ite_k == ite_k_total)
			//if((double)rand() / RAND_MAX < 0.5 && ite_k == ite_k_total)
			if (ite_k % 1 == 0 || ite_k == ite_k_total)
			//if ((double)rand() / RAND_MAX < 0.5)
			//if (ite_k % ite_k_total == 0)
			{
				ite++; ite_rg++;
				thermal_diffusivity_regularization.UpdateCurrentEta(current_eta_regularization);
				thermal_diffusivity_regularization.UpdateMaximumVariation(maximum_variation_current_global);
				thermal_diffusivity_regularization.UpdateAveragedVariation(averaged_variation_current_global);
				thermal_diffusivity_regularization.parallel_exec(dt);
		...
		//while ((averaged_residual_T_current_global > averaged_residual_T_last_global && averaged_residual_T_current_global > 0.000005) || ite_T < ite_T_total)
		//while (averaged_residual_T_current_global > averaged_residual_T_last_global || ite_T < ite_T_total)
		//while (averaged_residual_T_current_global > averaged_residual_T_last_global)
		//while (ite_T < ite_T_total)
		{
			...
			if (ite_T == 1)
			{
				...
				out_file_all_information << std::fixed << std::setprecision(12) << ite_loop << "   " << ite << "   " <<
					averaged_residual_T_current_local << "   " << averaged_residual_T_current_global << "   " << averaged_residual_k_current_local << "   " <<
					averaged_residual_k_current_global << "   " << averaged_variation_current_local << "   " << averaged_variation_current_global << "\n";
				out_file_residual_for_comparison << std::fixed << std::setprecision(12) << ite_T_comparison_opt << "   " << averaged_residual_T_current_global << "\n";*/
			}
		}
		/*out_file_error_plot << std::fixed << std::setprecision(12) << ite << "   " <<
			averaged_residual_T_current_global << "   " << maximum_residual_T_current_global << "   " <<
		...
		//decay learning rate by temperature to be zero
		//if (relative_difference < 0.00001)
		//{
		//	learning_rate_alpha = 0.95 * learning_rate_alpha;
		//	std::cout << "The learning rate is decreased by the observed parameters!" << std::endl;
		//}
